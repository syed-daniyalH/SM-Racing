...
class User(Base, TimestampMixin):
    ...
    role: Mapped[UserRole] = mapped_column(
        Enum(UserRole, name="user_role", schema=SM2RACING_SCHEMA),
        default=UserRole.MECHANIC,
        nullable=False,
    )
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    active_event_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey(f"{SM2RACING_SCHEMA}.events.id"), nullable=True)
