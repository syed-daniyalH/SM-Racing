...
class RunGroup(Base, TimestampMixin):
    ...
    event_id: Mapped[uuid.UUID] = mapped_column(ForeignKey(f"{SM2RACING_SCHEMA}.events.id"), unique=True, nullable=False)
    raw_text: Mapped[str] = mapped_column(String(255), nullable=False)
    normalized: Mapped[RunGroupCode] = mapped_column(
        Enum(RunGroupCode, name="run_group_code", schema=SM2RACING_SCHEMA),
        nullable=False,
    )
